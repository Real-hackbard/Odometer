...
type
  ...
  TssOdometer = class(TGraphicControl)
  private
  ...
  protected
    ...
    procedure SetRedDigitCount(Avalue: byte);
  ...
  published
    ...
    property RedDigitCount: byte read FRedDigitCount write SetRedDigitCount;
...
procedure Register;
begin
  RegisterComponents('Skybound Software', [TssOdometer]);
end;
...
begin
  ...
  if (iOld = 0) and ((iNew = MaxIndex) or (AValue = ALimit)) and (not AIncreasing) then 
  begin
    rDst.Left := 0;
  ...
  else if ((iOld = MaxIndex) or (AValue = 0)) and (iNew = 0) and AIncreasing then 
  begin
    rDst.Left := 0;
    rDst.Right := ADigits.Width;
    if AIncreaseTopToBottom then
...
begin
  ...
  for i := 0 to (FDigitCount - (1+FRedDigitCount)) do
  begin
    GetDigitImage(' ',
                  s[i+1],
                  FBitmap,
  ...
  for i := (FDigitCount-FRedDigitCount) to FDigitCount-1 do
  begin
    GetDigitImage(' ',
                  s[i+1],
                  FBitmap,
...
procedure TssOdometer.DrawAnimated(const AOldValue, ANewValue: string; AIncreasing: boolean);
var
  i: integer;
begin
  FFrameNo := 0;
...
procedure TssOdometer.StartTimerAni(const AOldValue, ANewValue: string; AIncreasing: boolean);
begin
  FFrameNo := 0;
  FOldValue := AOldValue;
  FNewValue := ANewValue;
...
procedure TssOdometer.TimerEvent(Sender: TObject);
var
  rSrc, rDst: TRect;
  i: integer;
begin
...
procedure TssOdometer.UpdateLimit;
begin
  case FBaseType of
    btBinary:
    begin
...
constructor TssOdometer.Create(AOwner: TComponent);
begin
  inherited;
  ...
  FRDigits := TBitmap.Create;
  FRDigits.LoadFromResourceName(HInstance, 'RDIGITS');
...
destructor TssOdometer.Destroy;
begin
  FTimer.Free;
  FDisplay.Free;
  FBitmap.Free;
...
procedure TssOdometer.Decrease;
var
  sOld: string;
begin
  FTimer.Enabled := False;
...
function TssOdometer.GetHeight: Integer;
begin
  Result := inherited Height;
end;
function TssOdometer.GetWidth: Integer;
begin
  Result := inherited Width;
end;
procedure TssOdometer.SetBaseType(AValue: TBaseType);
begin
...
procedure TssOdometer.SetDigitCount(AValue: byte);
begin
  if (AValue <> FDigitCount) and (AValue > 0) and (AValue <= 32) then
  begin
    FTimer.Enabled := False;
    FDigitCount := AValue;
    if FRedDigitCount > FDigitCount then
      FRedDigitCount := FDigitCount;
...
procedure TssOdometer.SetRedDigitCount(AValue: Byte);
begin
  if (AValue <> FRedDigitCount) and (AValue <= FDigitCount) then
  begin
    FTimer.Enabled := False;
    FRedDigitCount := AValue;
    UpdateLimit;
    SetValue(FValue);
  end;
end;
function TssOdometer.GetFrameInterval: byte;
begin
  Result := FTimer.Interval;
end;
procedure TssOdometer.SetFrameInterval(AValue: byte);
begin
  if AValue = 0 then
    inc(AValue);
...
procedure TssOdometer.SetValue(AValue: Cardinal);
begin
